...
def collate_tokens_with_word_dropout(
    values,
    pad_idx,
    eos_idx=None,
    left_pad=False,
    ...
    """Convert a list of 1d tensors into a padded 2d tensor with word dropout."""
    size = max(v.size(0) for v in values)
    size = size if pad_to_length is None else max(size, pad_to_length)
    if pad_to_multiple != 1 and size % pad_to_multiple != 0:
        size = int(((size - 0.1) // pad_to_multiple + 1) * pad_to_multiple)
    ...
    if word_dropout > 0.0 and src_dict is not None:
        for v in values:
            mask = torch.rand(v.size()) < word_dropout
            if dropout_method == 'unk':
                v[mask] = src_dict.unk()
            else:
                replacement = torch.randint(low=src_dict.nspecial, high=len(src_dict), size=v.size())
                v[mask] = replacement[mask]
